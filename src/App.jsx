...
const sb = {
  ...
  async get(table,params=""){ const r=await fetch(`${SUPABASE_URL}/rest/v1/${table}?${params}`,{headers:sb.headers}); if(!r.ok) throw new Error(await r.text()); return r.json(); },
  async post(table,body){ const r=await fetch(`${SUPABASE_URL}/rest/v1/${table}`,{method:"POST",headers:sb.headers,body:JSON.stringify(body)}); if(!r.ok) throw new Error(await r.text()); return r.json(); },
  async patch(table,id,body){ const r=await fetch(`${SUPABASE_URL}/rest/v1/${table}?id=eq.${id}`,{method:"PATCH",headers:sb.headers,body:JSON.stringify(body)}); if(!r.ok) throw new Error(await r.text()); return r.json(); },
  async delete(table,id){ const r=await fetch(`${SUPABASE_URL}/rest/v1/${table}?id=eq.${id}`,{method:"DELETE",headers:sb.headers}); if(!r.ok) throw new Error(await r.text()); },
};

const SITES = ["Burlington","Elkhorn","Lake Geneva","Whitewater","Delavan","Walworth","Genoa City","Twin Lakes"];
const SITE_COLORS = { Burlington:"#2563eb",Elkhorn:"#7c3aed","Lake Geneva":"#059669",Whitewater:"#d97706",Delavan:"#dc2626",Walworth:"#0891b2","Genoa City":"#db2777","Twin Lakes":"#65a30d" };
...
function parseTime(val){
  ...
  if(typeof val==="string"){const m=val.match(/(\d{1,2}):(\d{2})/);if(m)return`${String(m[1]).padStart(2,"0")}:${m[2]}`;const am=val.toLowerCase().match(/(\d{1,2})(?::(\d{2}))?\s*(am|pm)/);if(am){let h=parseInt(am[1]);if(am[3]==="pm"&&h!==12)h+=12;if(am[3]==="am"&&h===12)h=0;return`${String(h).padStart(2,"0")}:${am[2]||"00"}`;}return"07:00";}
...
function parseWorkbook(wb,knownUsers){
  ...
  wb.SheetNames.forEach(sheetName=>{
    ...
    const ws=wb.Sheets[sheetName];const rows=XLSX.utils.sheet_to_json(ws,{header:1,defval:""});if(rows.length<2)return;
    let headerIdx=0;for(let i=0;i<Math.min(5,rows.length);i++){if(rows[i].filter(Boolean).length>1){headerIdx=i;break;}}
    ...
    for(let r=headerIdx+1;r<rows.length;r++){
      const row=rows[r],rawDate=row[dateCol],rawName=String(row[nameCol]||"").trim();if(!rawDate||!rawName)continue;
...
function exportToExcel(shifts,users){
  ...
  SITES.forEach(site=>{const rows=[["Date","Staff Name","Start","End","Notes"]];shifts.filter(s=>s.site===site).sort((a,b)=>a.date.localeCompare(b.date)).forEach(s=>{const u=users.find(u=>u.id===s.user_id);rows.push([s.date,u?.name||s._importName||"Unknown",s.start_time,s.end_time,s.notes||""]);});const ws=XLSX.utils.aoa_to_sheet(rows);ws["!cols"]=[{wch:14},{wch:20},{wch:10},{wch:10},{wch:30}];XLSX.utils.book_append_sheet(wb,ws,site.substring(0,31));});
...
async function notifyStaffOfOpenShift(shift,users,publicLink){
  ...
  try{ await fetch(NOTIFY_ENDPOINT,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${SUPABASE_ANON_KEY}`},body:JSON.stringify({to:staffEmails,site:shift.site,date:formatDateLong(shift.date),start_time:shift.start_time,end_time:shift.end_time,notes:shift.notes,claim_url:publicLink})}); return true; }
  catch(e){ console.warn("Email notify failed:",e.message); return false; }
}
...
function Toast({msg,color="#1e3a5f"}){ if(!msg)return null; return<div style={{position:"fixed",top:70,right:20,zIndex:999,background:color,color:"#fff",borderRadius:10,padding:"12px 20px",fontSize:14,fontWeight:600,boxShadow:"0 4px 20px rgba(0,0,0,0.25)",maxWidth:380}}>{msg}</div>; }
function useToast(){ const[msg,setMsg]=useState("");const[color,setColor]=useState("#1e3a5f");const show=useCallback((m,c="#1e3a5f")=>{setMsg(m);setColor(c);setTimeout(()=>setMsg(""),4000);},[]);return[msg,color,show]; }
...
function AddToCalBtn({shift,userName,gcalConnected}){
  ...
  return<button onClick={handle} style={{padding:"7px 14px",borderRadius:8,border:"1px solid #4285f4",background:done?"#4285f4":"#fff",color:done?"#fff":"#4285f4",cursor:"pointer",fontSize:13,fontWeight:600,display:"flex",alignItems:"center",gap:5,transition:"all 0.2s",whiteSpace:"nowrap"}}>{done?"✓ Added!":<><img src="https://upload.wikimedia.org/wikipedia/commons/a/a5/Google_Calendar_icon_%282020%29.svg" alt="" style={{width:14,height:14}}/>{gcalConnected?"Add to Google Cal":"Download .ics"}</>}</button>;
}
function GCalBanner({gcalConnected,setGcalConnected}){
  ...
  return<><Toast msg={msg} color={clr}/><div style={{background:gcalConnected?"#f0fdf4":"#fffbeb",border:`1px solid ${gcalConnected?"#86efac":"#fcd34d"}`,borderRadius:12,padding:"14px 18px",marginBottom:20,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}><div style={{display:"flex",alignItems:"center",gap:12}}><img src="https://upload.wikimedia.org/wikipedia/commons/a/a5/Google_Calendar_icon_%282020%29.svg" alt="" style={{width:28,height:28}}/><div><div style={{fontWeight:700,fontSize:14,color:gcalConnected?"#166534":"#92400e"}}>{gcalConnected?"✓ Google Calendar Connected":"Connect Your Google Calendar"}</div><div style={{fontSize:12,color:gcalConnected?"#166534":"#92400e",opacity:0.8}}>{gcalConnected?"Shifts sync automatically — use your personal Gmail.":"Use your personal Gmail — no work IT needed."}</div></div></div>{gcalConnected?<button onClick={()=>setGcalConnected(false)} style={{padding:"7px 16px",borderRadius:8,border:"1px solid #86efac",background:"#fff",color:"#166534",cursor:"pointer",fontSize:13,fontWeight:600}}>Disconnect</button>:<button onClick={connect} disabled={syncing} style={{padding:"8px 18px",borderRadius:8,border:"none",background:"#4285f4",color:"#fff",cursor:"pointer",fontSize:13,fontWeight:700,opacity:syncing?0.7:1}}>{syncing?"Connecting…":"Connect Google Calendar"}</button>}</div></>;
}
...
// ── CalNav: shared prev/next for both week and month ───────────────────────
...
// ── MONTH CALENDAR ─────────────────────────────────────────────────────────
// Generic month grid — accepts shifts and a renderDay function for flexibility
function MonthGrid({monthDate, setMonthDate, shifts, renderCell, showNames=false, users=[]}) {
  ...
  // Build grid cells (pad start with nulls)
  const cells = [
  ...
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:2}}>
        {DAYS.map(d=>(
          <div key={d} style={{textAlign:"center",fontSize:11,fontWeight:700,color:"#6b7280",padding:"6px 0",background:"#f8fafc",borderRadius:4}}>
            {d}
          </div>
        ))}
      </div>
        ...
        {cells.map((dateStr,i)=>{
          ...
          return(
            <div key={dateStr} style={{
              ...
              <div style={{fontSize:11,fontWeight:700,color:isToday?"#2563eb":"#374151",marginBottom:3,textAlign:"right",paddingRight:2}}>
                {dayNum}
              </div>
  ...
  return(
    <div>
      ...
      {view==="week" ? (
        <>
          <CalNav
            label={`Week of ${formatDate(fmt(weekStart))}`}
...
function OpenShifts({user,gcalConnected,shifts,openShifts,setOpenShifts,setShifts,users}){
  ...
  const[msg,clr,show]=useToast();const[notifying,setNotifying]=useState(false);const[linkCopied,setLinkCopied]=useState(false);
  const publicLink=`${APP_PUBLIC_URL}/#open`;

  async function postOpen(){
    const newShift={...form,id:Date.now(),claimed_by:null};
    setOpenShifts(prev=>[...prev,newShift]);setForm(f=>({...f,notes:""}));
    setNotifying(true);const sent=await notifyStaffOfOpenShift(newShift,users,publicLink);setNotifying(false);
    show(sent?`✅ Open shift posted! Email sent to ${users.filter(u=>u.role==="staff").length} staff.`:"✅ Open shift posted! (Email needs Resend setup.)","#059669");
  }
  ...
  function claimShift(os,claimerName){
    setOpenShifts(prev=>prev.filter(s=>s.id!==os.id));setShifts(prev=>[...prev,{...os,id:Date.now(),user_id:user.id}]);
  ...
  return(
    <div>
      ...
      <div style={{background:"#f0f9ff",border:"1px solid #bae6fd",borderRadius:12,padding:16,marginBottom:20}}>
        ...
        <div style={{fontSize:13,color:"#0369a1",marginBottom:10}}>Hit <strong>Copy Shareable Link</strong> and text it — staff click it, see open shifts, and claim one without needing to log in.</div>
      ...
      {user.role==="admin"&&(
        <div style={{background:"#f0f9ff",border:"1px solid #bae6fd",borderRadius:12,padding:20,marginBottom:24}}>
          ...
          <button onClick={postOpen} disabled={notifying} style={{marginTop:14,padding:"10px 24px",borderRadius:8,border:"none",background:"#0369a1",color:"#fff",fontWeight:700,cursor:"pointer",fontSize:14,opacity:notifying?0.7:1,display:"flex",alignItems:"center",gap:8}}>{notifying?"Sending…":"📢 Post & Notify Staff"}</button>
        </div>
      )}
      {available.length===0
              ...
              <div key={s.id} style={{...card,borderLeft:`4px solid ${siteColor(s.site)}`,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  {user.role==="admin"
                    ...
                    :<button onClick={()=>claimShift(s,user.name)} style={{padding:"8px 18px",borderRadius:8,border:"none",background:"#059669",color:"#fff",cursor:"pointer",fontSize:13,fontWeight:700,display:"flex",alignItems:"center",gap:6}}>✋ Claim Shift {gcalConnected&&<img src="https://upload.wikimedia.org/wikipedia/commons/a/a5/Google_Calendar_icon_%282020%29.svg" alt="" style={{width:14,height:14}}/>}</button>
                  }
                </div>
              </div>
            );
...
function BySite({shifts,users}){
  ...
  function renderMonthCell(dateStr,dayShifts){
    return dayShifts.map((s,j)=>{
      const u=users.find(u=>u.id===s.user_id);
      return<div key={j} style={{background:"#1e3a5f",borderRadius:3,color:"#fff",padding:"2px 5px",fontSize:10,marginBottom:2,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"}}>{u?.name||"?"}</div>;
    });
  }
  ...
  return(
    <div>
      ...
      {view==="week" ? (
        ...
        <MonthGrid monthDate={monthDate} setMonthDate={setMonthDate} shifts={siteShifts} renderCell={renderMonthCell} users={users}/>
      )}
...
function ByLead({shifts,users}){
  ...
  const weekDays=Array.from({length:7},(_,i)=>fmt(addDays(weekStart,i)));
  const teamShifts=shifts.filter(s=>teamUsers.some(u=>u.id===s.user_id));

  function renderMonthCell(dateStr,dayShifts){
    return dayShifts.map((s,j)=>{
      const u=users.find(u=>u.id===s.user_id);
      return<div key={j} style={{background:siteColor(s.site),borderRadius:3,color:"#fff",padding:"2px 5px",fontSize:10,marginBottom:2,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"}}>{u?.name||"?"} · {s.site}</div>;
    });
  }
  ...
  return(
    <div>
      ...
      {view==="week" ? (
        ...
        <MonthGrid monthDate={monthDate} setMonthDate={setMonthDate} shifts={teamShifts} renderCell={renderMonthCell} users={users}/>
      )}
    </div>
  );
}
...
function ExcelPanel({onImport,shifts,users}){
  ...
  function processFile(file){
    ...
    reader.onload=e=>{try{const wb=XLSX.read(e.target.result,{type:"array",cellDates:false});const{shifts:parsed,warnings}=parseWorkbook(wb,users);const unmapped=[...new Set(parsed.filter(s=>!s.user_id).map(s=>s._importName))];setPreview({shifts:parsed,warnings,unmapped});setStatus("done");}catch(err){show("❌ Couldn't read that file: "+err.message,"#dc2626");setStatus("idle");}};
...
function PublicOpenShifts({openShifts,onClaim,gcalConnected}){
  const[name,setName]=useState("");const[claimed,setClaimed]=useState(null);
  const available=openShifts.filter(s=>!s.claimed_by&&s.date>=fmt(today));
  function claim(shift){if(!name.trim()){alert("Enter your name first!");return;}onClaim(shift,name.trim());setClaimed(shift);}
  if(claimed)return(
    <div style={{minHeight:"100vh",background:"#f0fdf4",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Segoe UI',sans-serif"}}>
      <div style={{background:"#fff",borderRadius:20,padding:48,maxWidth:460,textAlign:"center",boxShadow:"0 10px 40px rgba(0,0,0,0.1)"}}>
        ...
        <p style={{color:"#6b7280",marginBottom:24}}><strong>{name}</strong>, you're confirmed for <strong>{claimed.site}</strong> on <strong>{formatDateLong(claimed.date)}</strong>, {claimed.start_time}–{claimed.end_time}.</p>
        <button onClick={()=>exportICS([claimed],name)} style={{padding:"12px 28px",borderRadius:10,border:"none",background:"#4285f4",color:"#fff",fontWeight:700,cursor:"pointer",fontSize:15}}>⬇ Download to Calendar</button>
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0f2442 0%,#1e3a5f 100%)",fontFamily:"'Segoe UI',sans-serif",padding:"40px 16px"}}>
      <div style={{maxWidth:640,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:32}}><div style={{fontSize:44,marginBottom:8}}>🏥</div><h1 style={{color:"#fff",margin:0,fontSize:28,fontWeight:800}}>Open Shifts</h1><p style={{color:"rgba(255,255,255,0.7)",marginTop:8,fontSize:15}}>See an open shift you can cover? Enter your name and claim it.</p></div>
        ...
        {available.length===0?<div style={{background:"rgba(255,255,255,0.1)",borderRadius:16,padding:48,textAlign:"center",color:"rgba(255,255,255,0.7)",fontSize:16}}>🎉 No open shifts right now!</div>
          :available.map(s=>(
            <div key={s.id} style={{background:"#fff",borderRadius:16,padding:24,marginBottom:16,borderLeft:`5px solid ${siteColor(s.site)}`,boxShadow:"0 4px 20px rgba(0,0,0,0.15)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12}}>
                <div><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}><SiteBadge site={s.site}/><span style={{fontWeight:700,color:"#1e3a5f",fontSize:17}}>{formatDateLong(s.date)}</span></div><div style={{color:"#6b7280",fontSize:14,marginBottom:4}}>🕐 {s.start_time} – {s.end_time}</div>{s.notes&&<div style={{color:"#9ca3af",fontSize:13}}>📋 {s.notes}</div>}</div>
                <button onClick={()=>claim(s)} style={{padding:"12px 24px",borderRadius:10,border:"none",background:"#059669",color:"#fff",fontWeight:700,cursor:"pointer",fontSize:14,whiteSpace:"nowrap"}}>✋ I'll Take It</button>
              </div>
            </div>
          ))
        }
...
export default function App(){
  ...
  if(hash==="#open")return<PublicOpenShifts openShifts={openShifts} gcalConnected={gcalConnected} onClaim={(shift,name)=>setOpenShifts(prev=>prev.map(s=>s.id===shift.id?{...s,claimed_by:name}:s))}/>;
