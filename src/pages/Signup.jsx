...
import { signUp, savePendingInvite } from "../lib/api";

export default function Signup({ onGoLogin, banner, isInvite, inviteToken }) {
  ...
  async function handle() {
    ...
    try {
      ...
      // fires and App.jsx moves on to onboarding (accepting the invite, or
      // "set up your clinic" for a brand-new signup).
    } catch (e) {
    ...
    return (
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0f2442 0%,#1e3a5f 50%,#0f2442 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Georgia',serif" }}>
        <div style={{ background: "#fff", borderRadius: 20, padding: 40, width: "100%", maxWidth: 420, boxShadow: "0 30px 80px rgba(0,0,0,0.4)", textAlign: "center" }}>
          ...
          <p style={{ color: "#6b7280", marginTop: 10, fontSize: 14 }}>We sent a confirmation link to <strong>{email}</strong>. Click it, then come back here and sign in{isInvite ? " — you'll land right in your clinic's schedule." : " — we'll walk you through setting up your clinic."}</p>
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0f2442 0%,#1e3a5f 50%,#0f2442 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Georgia',serif" }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: 40, width: "100%", maxWidth: 420, boxShadow: "0 30px 80px rgba(0,0,0,0.4)" }}>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          ...
          <p style={{ color: "#6b7280", margin: "6px 0 0", fontSize: 14 }}>{isInvite ? "You've been invited — create a password to join your clinic's schedule." : "Set up ClinicSched for your clinic or practice."}</p>
        </div>
        ...
        <div style={{ display: "flex", borderRadius: 10, border: "1px solid #d1d5db", overflow: "hidden", marginBottom: 20 }}>
          <button onClick={onGoLogin} style={{ flex: 1, textAlign: "center", padding: "9px 0", background: "#fff", color: "#374151", fontWeight: 600, fontSize: 13, border: "none", borderRight: "1px solid #d1d5db", cursor: "pointer" }}>Sign In</button>
          <div style={{ flex: 1, textAlign: "center", padding: "9px 0", background: "#1e3a5f", color: "#fff", fontWeight: 700, fontSize: 13 }}>
